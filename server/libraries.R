...
libTable <- reactive({
  con %>% 
    ...
    dplyr::select("Guide-ID" = guide_id, "Entrez-ID" = entrez_id, "Gene-Symbol" = symbol, "Ensembl-ID" = ensembl_id, Sequence = sequence, Length, 
    ...
    dplyr::select(which(colMeans(!is.na(.)) > 0.5))
